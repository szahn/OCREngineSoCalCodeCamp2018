...
func (r *SourceDocument) ToImage(rasterizeOpts *DocumentRasterizeOptions) (string, []byte, error) {
			...
			case EngineGhostScriptLib:
				{
					...
					ext := "png"
					destPath := rasterizeOpts.OutputPath

					imageName := fmt.Sprintf("%s.%s", random.NextRandom(), ext)
					destFilename := fmt.Sprintf("%s/%s", destPath, imageName)

					fmt.Printf("Rasterizing image %s to %s\r\n", r.source, destFilename)
					...
					fmt.Printf("Rasterized document saved to %s\r\n", destFilename)
